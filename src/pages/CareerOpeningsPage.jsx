...
function CareerOpeningsPage({ type }) {
  ...
  useEffect(() => {
    ...
    if (cachedOpenings.has(cacheKey)) {
      if (active) {
        setState({
          loading: false,
          openings: cachedOpenings.get(cacheKey),
          error: "",
        });
      }
      return () => {
    ...
    const fetchPromise =
      pendingOpenings.get(cacheKey) ||
      getOpenings(type)
        .then((items) => {
          const filtered = items.filter((item) => item.type === type);
          cachedOpenings.set(cacheKey, filtered);
          pendingOpenings.delete(cacheKey);
          return filtered;
        })
        .catch((error) => {
          pendingOpenings.delete(cacheKey);
    ...
    pendingOpenings.set(cacheKey, fetchPromise);
    ...
    <main className={styles.page}>
      <section className={styles.hero}>
        <div className="container">
          <Link className={styles.back} to="/career">
            <ArrowLeft size={17} /> Back to careers
          </Link>
          <div className={styles.heroGrid}>
            ...
            <div
              className={styles.careerVisual}
              aria-label={`${content.title} career animation`}
            >
              ...
              <div className={styles.animationFrame}>
                <DotLottieReact
                  className={styles.careerAnimation}
                  src="https://lottie.host/edee592e-e1bd-477c-8cbc-19ce85174348/OKqBTBHfTD.lottie"
                  loop
                  autoplay
                />
              </div>
              <span className={styles.categoryBadge}>
                ...
                {type === "internship" ? "LEARN & BUILD" : "CREATE & LEAD"}
              </span>
              <span className={styles.signalBadge}>
                <Sparkles size={14} /> OPPORTUNITIES LIVE
              </span>
            </div>
          </div>
        </div>
      ...
      <section className={`${styles.listing} container`}>
        <header>
          ...
          <span>
            ...
            {state.openings.length === 1 ? "opening" : "openings"}
          </span>
        </header>
        {state.loading && (
          <div className={styles.state}>Loading current opportunities…</div>
        )}
        {state.error && (
          <div className={`${styles.state} ${styles.error}`}>{state.error}</div>
        )}
        {!state.loading && !state.error && state.openings.length === 0 && (
          <div className={styles.empty}>
            <div
              className={styles.emptyAnimationWrap}
              aria-label="No current openings animation"
            >
              <DotLottieReact
            ...
            <h3>
              No active{" "}
              {type === "internship" ? "internships" : "experienced roles"}{" "}
              right now.
            </h3>
            <p>
              We are always interested in meeting thoughtful people. Introduce
              yourself and we will keep you in mind.
            </p>
            <a
              href={applicationUrl(
                type === "internship" ? "Internship" : "Open opportunity",
              )}
              target="_blank"
              rel="noreferrer"
            >
              Introduce yourself <ArrowUpRight size={17} />
            </a>
          </div>
        )}
          ...
          {state.openings.map((opening) => (
            <article key={opening._id}>
              ...
              <div className={styles.vacancy}>
                <UsersRound size={15} />
                <strong>{Number(opening.vacancies) || 1}</strong>{" "}
              ...
              <Link to={`/career/position/${opening._id}`}>
                View position details <ArrowUpRight size={18} />
              </Link>
            </article>
          ))}
        </div>
